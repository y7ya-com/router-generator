...
var Generator = class Generator {
	...
	buildRouteTree(opts) {
		...
		else if (isSingleExportRouteFile(node.filePath)) virtualRouteNodes.push(`const ${node.variableName}RouteImport = createFileRoute('${node.routePath}')()`);
		...
		let hasComponentPieces = rootIsSfc || sortedRouteNodes.some((n) => isSingleExportRouteFile(n.filePath));
		...
		const rootComponentNode = rootPieces?.component ?? (rootIsSfc ? rootRouteNode : void 0);
...
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.`,
	...
	async processRouteNodeFile(node) {
		...
		if (!isSingleExportRouteFile(node.filePath)) {
